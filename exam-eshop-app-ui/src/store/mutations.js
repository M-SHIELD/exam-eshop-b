...
export default {
    addFooterPrint(state, item) {
        var flag = false
        state.FootPrint.forEach(e => {
            if (e.id === item.id) {//存在,更新时间
                flag = true
            }
        })
        if (flag === false)
            state.FootPrint.push(item)
    },
    removeFooterPrint(state, id) {
        var ind;
        for (let i = 0; i < state.FootPrint.length; i++) {
            if (state.FootPrint[i].id===id)
                ind=i
        }
        state.FootPrint.splice(ind,1)
    },
    // 保存用户信息至本地
    [USER_INFO](state, {
        userInfo
